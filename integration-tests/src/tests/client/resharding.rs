...
fn non_resharding_cross_contract_calls_impl(prob: f64, rng_seed: u64) {
    ...
    let genesis_protocol_version = 55; // PROTOCOL_VERSION; //get_genesis_protocol_version(&resharding_type);
    let target_protocol_version = 55; //PROTOCOL_VERSION; // get_target_protocol_version(&resharding_type);
    let mut test_env =
        create_test_env_for_cross_contract_test(genesis_protocol_version, epoch_length, rng_seed);
    ...
    let drop_chunk_condition = DropChunkCondition::new();
    for _ in 1..3 {
        test_env.step_impl(&drop_chunk_condition, target_protocol_version, None, true);
    }
