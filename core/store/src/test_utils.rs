...
use crate::{
    get, get_delayed_receipt_indices, get_promise_yield_indices, DBCol, LookupMode, NodeStorage, ShardTries, StateSnapshotConfig, Store, Trie, TrieConfig
};
...
pub fn test_populate_trie(
    ...
    let trie_changes = trie.update(changes.iter().cloned(), LookupMode::FLAT_STORAGE).unwrap();
    let mut store_update = tries.store_update();
    ...
    for (key, value) in deduped {
        assert_eq!(trie.get(&key, LookupMode::FLAT_STORAGE), Ok(value));
    }
    root
}
