...
use crate::{LookupMode, ShardTries, StateSnapshotConfig, Store, Trie, TrieConfig, TrieDBStorage, TrieStorage};
...
pub fn construct_trie_from_flat(store: Store, write_store: Store, shard_uid: ShardUId) {
    ...
    while let Some(batch) = get_trie_update_batch(&mut iter) {
        ...
        let trie_changes = new_trie.update(batch, LookupMode::DEFAULT).unwrap();
