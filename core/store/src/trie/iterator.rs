    ...
    fn has_value(&self) -> bool {
        match self.trail.last() {
            Some(b) => match (&b.status, &b.node.node) {
                ...
                (CrumbStatus::At, TrieNode::Leaf(_, _)) => false,
                _ => false,
            },
    ...
    pub fn visit_nodes_interval(
        ...
        loop {
            ...
            match iter_step {
                ...
                IterStep::Continue => {}
                IterStep::Value(hash) => {
                    if let Err(err) = self.trie.storage.retrieve_raw_bytes(&hash) {
                        tracing::debug!(target: "state-part", ?hash, ?err, "retrieve_raw_bytes failed");
                    }
                    nodes_list.push(TrieTraversalItem {
                        hash,
                        key: self.has_value().then(|| self.key()),
