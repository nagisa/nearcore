...
fn work(
    ...
    tracing::trace!(target: "my_trie", prefix, from, to);
    assert!(from < to);
    assert!(to <= items.len());
    // assert!(items[from].0[..prefix] == items[to - 1].0[..prefix]);
    let res = if from + 1 == to {
        if from % 3000000 == 0 {
            tracing::info!(target: "my_trie", ?prefix, ?from, ?to, key = ?NibbleSlice::nibbles_to_string(&items[from].0.iter().collect::<Vec<u8>>()));
        }
    ...
    tracing::trace!(target: "my_trie", prefix, from, to, hash = ?res.0, raw_node = ?res.2);
    res
}
