...
use near_store::{LookupMode, Trie};
...
fn trie_lookup(bench: &mut Bencher) {
    let (changed_keys, trie) = {
        ...
        let trie_changes = trie.update(changes, LookupMode::FLAT_STORAGE).unwrap();
    ...
    bench.iter(|| {
        for _ in 0..1 {
            for key in changed_keys.iter() {
                trie.get(key, LookupMode::FLAT_STORAGE).unwrap();
            }
        }
    });
}
...
fn trie_update(bench: &mut Bencher) {
    ...
    bench.iter(|| {
        let _ = trie.update(changes.iter().cloned(), LookupMode::FLAT_STORAGE);
    });
}
