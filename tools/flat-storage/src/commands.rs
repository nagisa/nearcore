...
enum SubCommand {
    ...
    ComputeStateRoot(ComputeStateRootCmd),
...
#[derive(Parser)]
pub struct ComputeStateRootCmd {
    #[clap(long)]
    shard_id: ShardId,
    #[clap(long)]
...
impl FlatStorageCommand {
    ...
    fn compute_state_root(
        ...
        let status: FlatStorageStatus = status.unwrap();
        let flat_head = match status {
            FlatStorageStatus::Ready(FlatStorageReadyStatus { flat_head }) => flat_head,
            _ => panic!("Unexpected"),
        };
    ...
    fn move_flat_head(
        ...
        flat_storage.update_flat_head(header.hash(), true).unwrap();
        Ok(())
    }
